...
const ENCRYPTION_KEY = process.env.FILE_ENCRYPTION_KEY ? Buffer.from(process.env.FILE_ENCRYPTION_KEY, "hex") : null; // 32 bytes for AES-256
...
export async function uploadFile(req, res) {
  try {
    ...
    const { roomCode, uploadedBy } = req.body;
    if (!roomCode)
...
export async function listFiles(req, res) {
  try {
    const files = await FileAsset.find({ roomCode: req.params.roomCode }).sort({
      createdAt: -1,
    });
...
export async function downloadFile(req, res) {
  try {
    ...
    const encrypted = await fs.readFile(
      path.resolve("uploads", asset.storedName),
    );
